...
Для этого приложения практичнее стандартный Python CLI-пакет, а не Docker или
единый исполняемый файл:

- одинаковая установка на macOS, Linux и Windows;
- простой запуск из virtualenv или через `pipx`;
- Codex CLI, авторизация и MCP-конфигурация остаются пользовательскими;
- обновление из GitHub не требует пересборки контейнера;
- голосовая расшифровка подключается отдельно и не утяжеляет базовую установку.
